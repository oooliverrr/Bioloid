/**
 *
 *   Bioloid Remote Brain (BRBrain) custom CM-5 firmware.
 *
 *******************************************************************************
 *
 ...
 *
 * $Id: brbrain.c 45 2012-07-26 18:45:57Z vona $
 **/
  ...
  if (rxPC(&addr, 1) && endRXPacketPC()) {
    ...
    if (addr <= 253) {
      ...
      do { /* retry loop */
        ...
        if (!endTXPacketDynamixel()) break;

      } while (rxPacketDynamixel(&rxID, 0, &rxError, 0, 0) &&
               tryDynamixelAgain(rxError));

      if (rxID != addr) setStatusFlag(S_INVALID_DYNAMIXEL_RESPONSE);
    }

    if (addr == 254) setStatusFlag(S_INVALID_PC_COMMAND);
...
static void handleSetReadFormat() { setFormat(F_READ); }

static void handleSetWriteFormat() { setFormat(F_WRITE); }
...
static void handleReadData() {
  ...
  for (uint8_t i = 0; i < numDynamixels[F_READ]; i++) {
    ...
    while (n > 0) {
      ...
      if ((start+thisN) == VIRTUAL_ERROR_REG_ADDR) { /* TBD >= ?? */
        returnError = 1;
        thisN--;
      }

      for (uint8_t j = 0; j < thisN; j++) dynamixelParams[j] = 0xff;
      ...
      do { /* retry loop */
        ...
        if (thisN > 0) { /* still need to read some non-virtual regs */
          
          startTXPacketDynamixel(id, 2, D_I_READ_DATA);
          txDynamixel(start, 1);
          txDynamixel(thisN, 1);
          
        } else { /* only reading the virtual reg (error) */
          
          startTXPacketDynamixel(id, 0, D_I_PING);
        }
        
        if (!endTXPacketDynamixel()) goto DONE;
 ...
 DONE: 
  ...
  while (bytesToGo) txPC(0xff, 1); /* TBD unreachable, but inf loop ?? */
...
static void handleWriteData() {
  ...
  for (uint8_t i = 0; i < numDynamixels[F_WRITE]; i++) {
    ...
    do {
      ...
      for (uint8_t j = 0; j < n; j++) txDynamixel(dynamixelParams[j], 1);
    ...
    if (rxID != id) setStatusFlag(S_INVALID_DYNAMIXEL_RESPONSE);
  }
...
static void endTXPacketPC() { txPC(~checksumTxPC, 0); }
...
static uint8_t startRXPacketDynamixel(uint8_t *id,
  ...
  for (uint8_t i = 0; i < 2; i++) {
    if (!rxDynamixel(&byte, 0)) return 0;
    if (byte != 0xff) setStatusFlag(S_INVALID_DYNAMIXEL_RESPONSE);
  }

  if (!rxDynamixel(&byte, 1)) return 0;
  if (id) *id = byte;
  
  if (!rxDynamixel(&byte, 1)) return 0;
  if (numParams) *numParams = byte-2;
...
static uint8_t endRXPacketDynamixel() {
  ...
  if (!rxDynamixel(&checksum, 0)) return 0;
...
static uint8_t rxPacketDynamixel(uint8_t *id,
  ...
  if (!startRXPacketDynamixel(id, &actualNumParams, error)) return 0;

  if (numParams) *numParams = actualNumParams;

  uint8_t byte;

  for (uint8_t i = 0; i < actualNumParams; i++) {
    if (!rxDynamixel(&byte, 1)) return 0;
    if (params && (i < numParamsExpected)) params[i] = byte;
  }

  if (!endRXPacketDynamixel()) return 0;
...
static void txDynamixel(uint8_t byte, uint8_t addToChecksum) {
  ...
  if (addToChecksum) checksumTxDynamixel += byte;
}
...
static void txPC(uint8_t byte, uint8_t addToChecksum) {
  ...
  if (addToChecksum) checksumTxPC += byte;
}
...
static uint8_t rxDynamixel(uint8_t *byte, uint8_t addToChecksum) {
  ...
  if (addToChecksum) checksumRxDynamixel += b;
...
static uint8_t rxPC(uint8_t *byte, uint8_t addToChecksum) {
  ...
  if (addToChecksum) checksumRxPC += b;
...
static void clearRxBufDynamixel() { CBUF_Init(dynamixelRxBuffer); }

static void clearRxBufPC() { CBUF_Init(pcRxBuffer); }
...
ISR(SIG_UART0_RECV) {
  ...
  if (!CBUF_IsFull(dynamixelRxBuffer)) { CBUF_Push(dynamixelRxBuffer, byte); }
  else { status |= S_DYNAMIXEL_RX_OVERFLOW; LED_ON(LED_ERROR); }
}

/* tx buf to dynamixel empty */
ISR(SIG_UART0_DATA) {

  if (CBUF_IsEmpty(dynamixelTxBuffer)) { UCSR0B &= ~_BV(UDRIE0); }
  else { UDR0 = CBUF_Pop(dynamixelTxBuffer); }
}
...
ISR(SIG_UART1_RECV) {
  ...
  if (!CBUF_IsFull(pcRxBuffer)) { CBUF_Push(pcRxBuffer, byte); }
  else { status |= S_PC_RX_OVERFLOW; LED_ON(LED_ERROR); }
}

/* tx buf to pc empty */
ISR(SIG_UART1_DATA) {

  if (CBUF_IsEmpty(pcTxBuffer)) { UCSR1B &= ~_BV(UDRIE1); }
  else {
...
ISR(SIG_OUTPUT_COMPARE0) {
  ...
  if (ticksToGo) ticksToGo--;

  if (CBUF_IsEmpty(pcRxBuffer)) LED_OFF(LED_RXD_PC);
  if (CBUF_IsEmpty(pcTxBuffer)) LED_OFF(LED_TXD_PC);

  if (!BLUETOOTH_ACTIVE) {
    
    if (CBUF_IsEmpty(dynamixelRxBuffer)) LED_OFF(LED_RXD_DYNAMIXEL);
    if (CBUF_IsEmpty(dynamixelTxBuffer)) LED_OFF(LED_TXD_DYNAMIXEL);

  }  else {

    if (PORTD&_BV(BIT_BLUETOOTH_RTS)) LED_ON(LED_BLUETOOTH_RTS);
    else LED_OFF(LED_BLUETOOTH_RTS);
